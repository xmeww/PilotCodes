...
function [RT,Resp,keyIsDown,vbl,figframe] = present_outCyc(vbl,flipOnset,w,xy,tstart,paBG,fixbars,linewidth,white,f3,clr,tarIntv,ifi,paFG,Resp,RT,j,quit,figPresent,syncTest,fgStart,figOnly) 
   if figPresent ==1 || syncTest == 1
       
%        PsychPortAudio('UseSchedule', paFG, 3)
        pre_silience = fgStart(j,1)-1;
        PsychPortAudio('Start', paFG,1,vbl+pre_silience*3*ifi++0.01935);%+0.01955
...
%         if any(iframe == (tarIntv+async)*3-2) && (figPresent ==1 || syncTest ==1)
% %             intv
% 
