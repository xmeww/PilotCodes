function [ntrials,AVasync,figOnly,figPresent,nItems,Coh,tarOris,tarLocs,condTable,balTable,FGth] = M_cond_SFGfigOnlyAsync(condRep,trOrd,nItem_levels,coh_levels,AVsync_levels)
%% check func
% condRep = 48*2;% 24*2;
...
%% 
nconds = length(nItem_levels)*length(coh_levels)*length(AVsync_levels)+length(nItem_levels)*length(AVsync_levels)+length(nItem_levels);  
nfig = 4;
%%
...
[nitem11,coh11,avsync11,tarOris11, tarLocs11] = BalanceFactors(rep1,0,nItem_levels(1),coh_levels(1),...
    AVsync_levels,tarOri_type,1:tarLoc_level(1));
[nitem12,coh12,avsync12,tarOris12, tarLocs12] = BalanceFactors(rep2,0,nItem_levels(2),coh_levels(1),...
    AVsync_levels,tarOri_type,1:tarLoc_level(2));

figOnly_ord = [zeros(size([nitem1;nitem2]));ones(size([nitem11;nitem12]));...
    zeros(size([nitem3;nitem4]))];
figpresent_ord = [ones(size([nitem1;nitem2;nitem11;nitem12]));zeros(size([nitem3;nitem4]))];


%%
condTable_ord = array2table([cat(1,[nitem1,coh1,avsync1],[nitem2,coh2,avsync2],...
    [nitem11,coh11,avsync11],[nitem12,coh12,avsync12],...
...
%% 
L = [length(nitem1),length(nitem2),length(nitem11),length(nitem12)];
rep = [rep1 rep2 rep1 rep2];
fgth = [];%nan(nconds-2,length(fig_ord));

for i = 1:length(rep)
    fig_ord = repelem(1:nfig,L(i)/rep(i)/length(AVsync_levels)/nfig);

    for n = 1:length(AVsync_levels)*rep(i)
...
balTable_ord = array2table([cat(1,[tarOris1, tarLocs1],[tarOris2, tarLocs2],...
    [tarOris11, tarLocs11],[tarOris12, tarLocs12],...
