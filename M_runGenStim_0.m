...
elseif strcmp(paradigm,'Vselect_AsyncTarDis')
    [ntrials,nblocks,syncVTar,Vpresent,nItems,Apresent,Coh,tarOris,tarLocs,disLocs,nItemsChg,conds] = M_cond_Vselect_AsyncTarDis_3rand(nIntv,ncycle,nitemChg_type,condRep,trOrd,nItem_levels,coh_levels,blockRep,answer.pC);
end
...
%     S(b).intervals = intervals;
    ...
    S(b).intervals = intervals;
    if all(syncVTar(:,b) == 1)
        [XY,tarCenterXY,tarHemi,testcircle] = M_coordinate_0(V.Rroute,V.Lbar,V.xpixels,V.ypixels,V.tilt,vpresent,ntrials,nItem,tarLoc,tarOri,paradigm);
    ...
    elseif all(syncVTar(:,b) == 0)        
        [XY,tarCenterXY,tarHemi,testcircle,discircle] = M_coordinate(V.Rroute,V.Lbar,V.xpixels,V.ypixels,V.tilt,vpresent,ntrials,nItem,tarLoc,tarOri,paradigm,disLocs(:,b));
       ...
       % 
    if all(syncVTar(:,b) == 1)
    [clr_cyclewise,clr_trialwise, clrCodes_trialwise] = M_color_0(tarLoc,tarIntvs_trialwise,ntrials,ncycle,nIntv,nItem,nItemChange,...
        clr1,clr2,vpresent);
    elseif all(syncVTar(:,b) == 0)
    [clr_cyclewise,clr_trialwise, clrCodes_trialwise] = M_color_syncDis(tarLoc,disLocs(:,b),tarIntvs_trialwise,disIntvs_trialwise,ntrials,ncycle,nIntv,nItem,nItemsChg,clr1,clr2,vpresent);
    else 
           error('ERROR!!!not block design, color can not be generated with current script')
           return
    end
...
%         S(b).Sounds = Sounds;
       ...
       if all(syncVTar(:,b) == 1)
            [soundCycle,sounds,figCycle,fig,bgCycle,bg,nchords_segtrial,coh_comp,BG_comp] = M_sounds_Tekirandbg(ntrials,ncycle,A,apresent,coh,nbg_levels,tarIntvs_trialwise,fIntvs_trialwise);
       elseif all(syncVTar(:,b) == 0)
            [soundCycle,sounds,figCycle,fig,bgCycle,bg,nchords_segtrial,coh_comp,BG_comp] = M_sounds_Tekirandbg(ntrials,ncycle,A,apresent,coh,nbg_levels,disIntvs_trialwise,fIntvs_trialwise);
        
       else 
           erro('ERROR!!!not block design, sound can not be generated with current script')
           return
       end
       Sounds.sounds = sounds;
        Sounds.soundCycle = soundCycle;
        ...
        S(b).Sounds = Sounds; 
%     else
%         [figCycle,soundCycle,fig,sounds,nchords_segtrial] = M_sounds(ntrials,ncycle,A,Coh,Nbg,tarIntvs_trialwise,fIntvs_trialwise);
%    
%         Sounds.sounds = sounds;
%         Sounds.soundCycle = soundCycle;
%         Sounds.fig = fig;
...
Tr.nIntv = nIntv ;
if ~strcmp(paradigm,'Aselect01_singleV01');
    Tr.nItems = nItems ;Tr.nItemsChg = nItemsChg;Tr.tarLocs = tarLocs;
else
    Tr.nItems = ones(ntrials,nblocks) ;Tr.nItemsChg = [];Tr.tarLocs = [];
end
