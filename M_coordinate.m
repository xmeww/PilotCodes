function [XY,disOri,Ori,tarCenter,fixbars,tarMarker]=M_coordinate(monitor,monitorMode,tarOris,tarLocs,tilt,ntrials,nItems)
%%
% monitor = 'lab1'; %lab1,mac
% monitorMode =  'plain';%'palin','small','transparent','screen-short'
% tarOris =  Tr.tarOris;
% tarLocs = Tr.tarLocs;
% tilt = 15;
% ntrials = Tr.ntrials;
...
% tarOris = repmat([0,1]',5,1);
% tarLocs = [1:10]';
% tilt = 15;
% ntrials = 10;
% nItems = repmat([12,14]',5,1);
...
Rroute= M_deg2pix(vdist,VARcircle,xpixels,cwidth); % 203.066 195.87
Lbar= M_deg2pix(vdist,VAbar,xpixels,cwidth); % 20.985 19.077
%% ---------  ( *・ω・)✄ soul ------------
Lfix = Lbar;
...
%             end
    ...
    tarMarker (j,:) = [tarCenter(1,j)-sqrt(3)*Lbar/2,tarCenter(2,j)-sqrt(3)*Lbar/2,tarCenter(1,j)+sqrt(3)*Lbar/2,tarCenter(2,j)+sqrt(3)*Lbar/2];
    XY(:,:,j) = {[x;y]};
end
...
%     
    Screen('DrawLines', w, XY{j}, 3,white);
    Screen('Flip',w,[],1)
