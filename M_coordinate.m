...
%% input parameters
% clear all;
function [XY,tarCenterXY,tarHemi,testcircle,discircle] = M_coordinate(Rroute,Lbar,xpixels,ypixels,tilt,Vpresent,ntrials,nItem,tarLoc,tarOri,paradigm,disLoc)
...
%             y(ind+1) = centery(i) - Lbar/2*sin(templine1*pi/2+templine2*tilt/180*pi);
    ...
    % center coordinate of bars 
        if vpresent ==1
            ...
            disloc = disLoc(j);
            disCenterX = centerx(disloc);
            disCenterY = centery(disloc); 
            discircle(j,:) = [disCenterX - sqrt(2)*Lbar/2, disCenterY - sqrt(2)*Lbar/2,disCenterX + sqrt(2)*Lbar/2,disCenterY + sqrt(2)*Lbar/2];
