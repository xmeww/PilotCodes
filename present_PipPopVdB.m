        ...
        elseif ~keyIsDown && iframe== numFrames
           ...
           while ~keyIsDown
               ...
               [RT,Resp,keyIsDown,vbl,figframe] = present_outCyc(vbl,flipOnset,w,XY{j},tstart(j),paBG,...
                   fixbars,linewidth,white,f3,clr_out,tarIntv_tr(j,:),ifi,paFG,Resp,RT,j,quit,Tr.figPresent(j),syncTest,fgStart);
           end
           break
        end
...
%     end
    ...
    save(['data-',datestr(now,'mm_dd_HH_MM'),'.mat'],'RT', 'Resp','cond', 'keyH', 'keyV')
    sca;
    PsychPortAudio('Close',[])
    ShowCursor;
    ListenChar(0);
