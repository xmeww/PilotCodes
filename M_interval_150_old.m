function [tarIntv_c, tarIntv_tr, tar_dist, nchg, disLocs, item_intv, fgStart,closeTar,chkItemChg] = M_interval_150(ntrials,tarLocs,nItems,ncycle,nintv_tr,nintv_c,AVasync,figPresent,preCyc_int)
...
% nintv_c = 18; %  0.9/0.05
% AVasync = [0 0 0 0];%Tr.AVasync; % 0 sync, acync -1 = Apresent 0 
% preCyc_int = 500/50;
% figPresent = [1 1 1 1];
...
% ------------------- 200 -Tar-200 -------------------
tpw = 150; % temporal protected window, in ms
...
nchg(j,find(isnan(nchg(j,:)))) = randi(length(nchg_type),size(find(isnan(nchg(j,:)))))-1;% 0~3 distr change 
% h = histogram(d);
% h.Normalization = 'probability';
% set(gca,'XLim',[0.5,max(d+0.5)],'XTick',[1:max(d)],'XTickLabel',{'50','100','150','200','250','300','350'})
